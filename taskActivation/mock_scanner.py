...
cadence, exactly matching the project's dicomNamePattern, so you can test the
full real-time DICOM streaming path (file-watch -> getIncremental -> analysis)
without a real scanner. RT-Cloud converts each mosaic to a NIfTI with dcm2niix.

Volume source:
  --source synthetic   (default) build a 64x64x27 series with task activation
                       driven by the events file (condA / condB regions).
  --source <nifti>     replay a real 4D NIfTI, resampling each volume to 64x64x27.
...
TEMPLATE = os.path.join(HERE, 'templates', 'mosaic_template.dcm')
MOSAIC_SLICES = 27          # matches the template's CSA NumberOfImagesInMosaic
TILE = 64                   # in-plane tile size (Rows/ncols); template is 6x6x64
...
def synthetic_series(events, nVols, TR, condA, condB, shape=(TILE, TILE, MOSAIC_SLICES),
...
def nifti_series(path, nVols_hint=None, shape=(TILE, TILE, MOSAIC_SLICES)):
    """Load a 4D NIfTI and resample each volume to the mosaic grid (64x64x27)."""
...
def write_dicom(template, vol3d, out_path, instance, run):
    import pydicom
    mos = mrt.volume_to_mosaic(np.clip(vol3d, 0, 65535).astype(np.uint16))
    ds = copy.deepcopy(template)
    ds.Rows, ds.Columns = mos.shape                       # 384 x 384 (unchanged)
    ds.PixelData = mos.astype('<u2').tobytes()
    ds.InstanceNumber = int(instance)
    ds.AcquisitionNumber = int(instance)
    ds.SeriesNumber = int(run)
    ds.SOPInstanceUID = pydicom.uid.generate_uid()
    tmp = out_path + '.part'
    ds.save_as(tmp)                                       # write fully, then rename
...
def main(argv=None):
    ...
    ap.add_argument('--tr', type=float, default=None, help='seconds between volumes (default from config demoStep)')
    ...
    TR = args.tr if args.tr is not None else float(cfg.get('demoStep', 2.0))
    ...
    if args.source == 'synthetic':
        ...
        series, _ = synthetic_series(events, nVols, TR, condA, condB)
        print(f"[mock] synthetic {task}: {nVols} vols, condA={condA} condB={condB}")
    else:
        series, _ = nifti_series(args.source)
        nVols = args.nvols or series.shape[3]
        print(f"[mock] NIfTI replay: {args.source} -> {nVols} vols (resampled to 64x64x27)")

    try:
        import pydicom
    except Exception:
        print("[mock] pydicom is required (pip install pydicom)"); return 1
    template = pydicom.dcmread(TEMPLATE)
    ...
    for k in range(nVols):
        ...
        write_dicom(template, series[..., k], out_path, instance=idx, run=run)
